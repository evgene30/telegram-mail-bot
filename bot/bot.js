...
module.exports = function botControl() {
            ...
            try {
                await ctx.replyWithHTML(
                    ...
                    Markup.inlineKeyboard([
                        [Markup.button.callback("Restart bot", "restart"), Markup.button.callback("Help commands", "help")],
    ...
    bot.command("sendmail", async (ctx) => {
        try {
            await ctx.reply("Send new email?", Markup.inlineKeyboard([
                [Markup.button.callback("Send email", "send")]
            ]))
        } catch (e) {
            console.error(e)
                    ...
                    case "send":
                        ...
                        bot.on("text", async (ctx) => {
                            if (ctx.message.text.length > 3) {
                                ...
                                ctx.reply("Error: name less than 3 characters...")
                            }
                        })
